...
int32_t xvalue_new_char(const char *s, uint8_t **out) {
    if (!s || !*s) return xvalue_encode(XK_CHAR, NULL, 0, 0, out);
    ...
    int32_t r = xvalue_encode(XK_CHAR, raw, n * 4, n, out);
    free(raw);
    return r;
}
...
int32_t xvalue_new_char_utf8(const char *s, uint8_t **out) {
    if (!s) s = "";
    int32_t slen = (int32_t)strlen(s);
    return xvalue_encode(XK_CHAR_UTF8, (const uint8_t *)s, slen, slen, out);
}

int32_t xvalue_new_char_ascii(const char *s, uint8_t **out) {
    if (!s) s = "";
    int32_t slen = (int32_t)strlen(s);
    return xvalue_encode(XK_CHAR_ASCII, (const uint8_t *)s, slen, slen, out);
}
...
int32_t xvalue_new_ptr(const char *kind, const char *target, int32_t array_len, uint8_t **out) {
    if (!target) return -1;
    return xvalue_encode_ptr(kind, (const uint8_t *)target, (int32_t)strlen(target), array_len, out);
}
...
int32_t xvalue_new_extindex(const char *extpath, const char **children, int32_t count, uint8_t **out) {
    ...
    int32_t r = xvalue_encode(XK_EXT_INDEX, raw, (int32_t)pos, 1, out);
    free(raw);
    return r;
}
